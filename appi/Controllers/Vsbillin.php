<?php
	...
	class Vsbillin extends Controllers{
		...
		public function setVsbillin()
		{
			...
			$request_Vsbillin 	= $this->model->insertVsbillin($intPerios, $intStd_no, $strPer_no, $strCltype, $strRuc_no, $strRazons, $strDirecc, $strTlf_no, $strEmails, $intAbotyp, $strDoctip, $intDocval, $intDocabo, $datFecemi, $strPayfor, $strBan_no, $strChenum, $strDepnum, $strTar_no, $strTarnum, $strVounum);
			...
			if($request_Vsbillin > 0)
			{
				...
				$arrResponse = array('status' => false, 'msg' => 'ABONO está Incorrecto.');
...
//			$strCre_no    = strClean($_POST['listCre_no']);
			...
			if($request_Vsnotcre > 0){
				...
				$arrResponse = array('status' => false, 'msg' => 'No. Factura aplicada no existe.');
			}else{
				$arrResponse = array('status' => false, 'msg' => 'No es posible almacenar la información.');
			}
			echo json_encode($arrResponse,JSON_UNESCAPED_UNICODE);
