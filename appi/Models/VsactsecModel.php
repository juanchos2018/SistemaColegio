 <?php
	...
	class VsactsecModel extends Mysql
	{
		...
		public function getVsstdpro(int $stdid, int $stdperios, int $condition)
		{
			$this->intStd_id        = $stdid;
			$this->intStd_perios    = $stdperios;
			...
			switch ($condition)
			{
				case 1:
						...
						$sql = "SELECT STD_NO as codeStd,truncate(avg(a.Q1_PRO),2) as promedio
        	    		        ...
        	    		        WHERE a.STD_NO = {$this->intStd_id}
								AND a.PERIOS = {$this->intStd_perios}
            	        		AND m.REGIME = 1
        	        	    	AND m.CALIFI = 1";
		...
		public function getVssecdet(int $sec_no, int $perios, string $parcia, string $codigoAMIE)
		{
			$request = array();
			...
			// DATA EMPRESA
			$sql 				= "SELECT * FROM vsdefaul WHERE AMI_ID = '$codigoAMIE'";
			$request_empresa 	= $this->select($sql);

			//Se extrae nombres de las materias en la tabla: VSMATSTD
			$sql = "SELECT DISTINCT m.MAT_NM, s.SEC_NM, s.PARALE, s.JOR_NO
					FROM vsmatstd v
			...
			$CALIFICACION = "m.CUAL01,m.CUAL02,m.CUAL03,m.CUAL04,m.CUAL05,m.CUAL06,m.CUAL07,
			                 m.CUAL08,m.CUAL09,m.CUAL10,m.CUAN01,m.CUAN02,m.CUAN03,m.CUAN04,
			                 m.CUAN05,m.CUAN06,m.CUAN07,m.CUAN08,m.CUAN09,m.CUAN10 ";
		...
		public function insertVsactsec(int $niv_no, string $parale, string $sec_nm, int $pabell, int $modoes, int $regime, int $jor_no)
		{
		}
	}
