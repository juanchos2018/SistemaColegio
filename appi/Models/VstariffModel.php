	...
	class VstariffModel extends Mysql
	{
		...
		public function oneVstariff(int $idSTD)
		{
			...
			$sql = "SELECT 	v.SEC_ID,
			        ...
			        FROM vstariff v
			        INNER JOIN vstudent e ON v.STD_NO = e.STD_NO
			        INNER JOIN vsection s ON e.SEC_NO = s.SEC_NO ".$where;
		...
		public function updateVstariff(int $sec_id, int $perios, int $std_no, string $per_no, int $art_no, string $remark, float $docval, float $facval, float $aboval, string $replic)
		{
			...
			// Valida que la TARIFA no sea MAYOR al PRECIO DE LISTA
			if($this->intFacval > $this->intDocval or $this->intFacval < $this->intAboval or $this->intFacval < 0)
			{
				// VALOR INCORRECTO
				$return = -2;
				return $return;
			...
			if($this->intReplic == 'on')
			{
				...
				if(!empty($request_vsproduc))
				{
					for ($j = 0; $j <= 13; $j++) 
					{
						...
						if($this->strPer_no == "on")
						{
							...
							{
								$insert         = "UPDATE vstariff SET remark = ?, facval = ? WHERE PERIOS = {$this->intPerios} AND STD_NO = {$this->intStd_no} AND PER_NO = '{$this->strPer_no}' AND ART_NO = {$this->intArt_no}";
				...
				$insert         = "UPDATE vstariff SET remark = ?, facval = ? WHERE SEC_ID = {$this->intSec_id}";
				$arrData        = array($this->strRemark,$this->intFacval);
				$request_insert = $this->update($insert,$arrData);
				$return         = $request_insert;
			}
