 <?php
	...
	class VsbillinModel extends Mysql
	{
		...
		public function insertVsbillin(int $perios, int $std_no, string $per_no, string $cltype, string $ruc_no, string $razons, string $direcc, string $tlf_no, string $emails, int $abotyp, string $doctip, float $docval, float $docabo, string $fecemi, string $payfor, string $ban_no, string $chenum, string $depnum, string $tar_no, string $tarnum, string $vounum)
		{
			...
			{
				$insert         = "INSERT INTO vsbillin(perios,std_no,cltype,ruc_no,razons,direcc,tlf_no,emails,doctip,docval,docpto,docnum,fecemi,aut_no,ptoapl,docapl,docsig,logfil) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				$arrData        = array($this->intPerios,$this->intStd_no,$this->strCltype,$this->strRuc_no,$this->strRazons,$this->strDirecc,$this->strTlf_no,$this->strEmails,$this->strDoctip,$this->intDocabo,$this->strPto_no,$this->intDocnum,$this->datFecemi,$this->strAut_no,'',0,$this->intDocsig,$this->strLogfil);
				$request_insert = $this->insert($insert,$arrData);
				$return         = $request_insert;
			}else{
				// EXISTE
			...
			// Obtiene registros que van a ser facturados
			if ($this->intAbotyp == 1)
			{
