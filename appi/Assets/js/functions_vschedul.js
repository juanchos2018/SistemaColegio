...
$(document).ready(function() {
        ...
        TablaVschedule = $('#TableVschedul').DataTable({
        ...
        columnDefs: [
                ...
                { 'className': "anchocelda", "targets": [ 9 ]},
                { 'className': "anchocelda", "targets": [ 10 ]}
        ],
        buttons: [
            {
                "extend": "excelHtml5",
		...
		request.onreadystatechange = function()
   		{
   			...
   			{					 
   				...
   				if(objData.status)
   				{
                    ...
                    TablaVschedule.ajax.reload(null,false);
   				}else{
    ...
    request.onreadystatechange = function()
    {
        if(request.readyState == 4 && request.status == 200)
        {
            ...
            if(objData.status)
            {
                ...
                let $inst_codAMIE = '17H02852';
                let $inst_periodo = '2021';
    ...
    request.onreadystatechange = function()
    {
        if(request.readyState == 4 && request.status == 200)
        {
            ...
            {   
                ...
                // COMBO: Asignatura
                $opcion = objData.data.MAT_NO;
                document.querySelector('#listMat_no').value = $opcion;
                ...
                // Puntaje
                document.querySelector('#txtPuntaj').value = objData.data.PUNTAJ;

                // Fecha de Registro
                document.querySelector('#datFecreg').value = objData.data.FECREG;

                // Fecha de Cumplimiento
                document.querySelector('#datFecmax').value = objData.data.FECMAX;
                ...
                // COMBO: Insumo
                $opcion = objData.data.INSUMO;
                document.querySelector('#listInsumo').value = $opcion;
                $('#listInsumo').selectpicker('render');
                ...
                // Link Video
                document.querySelector('#txtVdlink').value = objData.data.VDLINK;
                ...
                if(objData.data.FLNAME != "")
                {
                    document.querySelector('.lblTask').classList.remove("notBlock");
                    document.querySelector('.lblNameTask').classList.remove("notBlock");
                    document.querySelector('.lblNameTask').innerHTML = objData.data.FLNAME;
                    document.querySelector('#txtNameTask').value = objData.data.FLNAME;
                }

                // Link descarga Tarea del Estudiante
                document.querySelector('.lblTaskStd').classList.add("notBlock");
                document.querySelector('.lblNameTaskStd').classList.add("notBlock");
                if(objData.data.FLTASK != "")
                {
                    let $inst_codAMIE = '17H02852';
                    let $inst_periodo = '2021';
                    let $descarga = '<a title="Descargar Archivo" href="Activity/'+$inst_codAMIE+'/'+$inst_periodo+'/upload/'+objData.data.FLTASK+'" download="'+objData.data.FLTASK+'" style="color: blue; font-size:14px;"> <i class="fas fa-download"></i> </a>';
                    ...
                    document.querySelector('.lblNameTaskStd').innerHTML = objData.data.FLTASK + ' ' +$descarga;
                }
