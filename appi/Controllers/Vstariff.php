	...
	class Vstariff extends Controllers{
		...
		public function getVstariffs()
		{
			...
			for ($i = 0; $i < count($arrData); $i++) 
			{
				...
				$btnPrnVstariff = '<a '.$color_boton_Prn.' title= "Imprimir Estado de Cuenta" href="'.base_url().'Vstariff/prnEstadoCuenta/'.$arrData[$i]['SEC_ID'].'"><i class="fas fa-print"></i></a>';
		...
		public function getVstariff(int $idSTD)
		{
			...
			if($intSTD > 0)
			{
				...
				if(empty($arrData))
				{
					$arrResponse = array('status' => false, 'msg' => 'Convenio no encontrado.');
				}else{
			...
			if($request_Vstariff > 0)
			{
				...
				switch($request_Vstariff)
				{
					case -1:
							...
							break;
					case -2:
							$arrResponse = array('status' => false, 'msg' => 'TARIFA posee valor Incorrecto.');
							break;
					default:
